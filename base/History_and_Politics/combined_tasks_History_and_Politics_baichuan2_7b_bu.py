...
def process_task(input_file_path, output_file_path, model, tokenizer, device, query_template):
    ...
    with open(input_file_path, 'r', encoding='utf-8') as infile, open(output_file_path, 'w', encoding='utf-8') as outfile:
        for line in infile:
            ...
            gen_kwargs = {"max_new_tokens": 2500, "repetition_penalty": 1.1}
            with torch.no_grad():
...
def main():
    model_path = "/public/chengweiwu/models/cdtp/Baichuan2-7B-Base"
    configure_logging('/public/chengweiwu/mingyang/logs/1115/combined_tasks_History_and_Politics_baichuan2_7b_bu.log')

    tokenizer, model, device = load_model(model_path)

    tasks = [
        {
            'input_file': '/public/chengweiwu/mingyang/data/input_data/submit/qa_submit/qa_all_Humanities_and_Society_random_1w1_head1w_dev2k.jsonl',
            'output_file': '/public/chengweiwu/mingyang/data/output_data/1115/qa_output_Humanities_and_Society_baichuan2_7b.jsonl',
            'template': qa_template},
        {
            'input_file': '/public/chengweiwu/mingyang/data/input_data/submit/kg_submit/kg_all_Nature_and_Environment_random1w1_head1w_dev2k.jsonl',
            'output_file': '/public/chengweiwu/mingyang/data/output_data/1115/kg_output_Nature_and_Environment_baichuan2_7b.jsonl',
            'template': kg_template},
        {
            'input_file': '/public/chengweiwu/mingyang/data/input_data/submit/textGen_submit/textGen_all_History_and_Politics_random_1w1_head1w_dev2k.jsonl',
            'output_file': '/public/chengweiwu/mingyang/data/output_data/1115/text_gen_output_History_and_Politics_baichuan2_7b.jsonl',
            'template': text_gen_template}
    ]

    # 使用 ThreadPoolExecutor 并行运行所有任务
    with ThreadPoolExecutor(max_workers=4) as executor:
        results = []
        for task in tasks:
            results.append(
                executor.submit(process_task, task['input_file'], task['output_file'], model, tokenizer, device,
                                task['template']))

        # 等待所有任务完成并处理结果
        for result in results:
            try:
                result.result()  # 意外的错误将在这里抛出
